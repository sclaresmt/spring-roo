 ...
 * @since 1.1
 */
public final class XmlRoundTripJsoupUtils {

  private static MessageDigest digest;

  private static String hashCodeAttrName = "z";
  ...
  private static boolean addOrUpdateElements(final Element original, final Element proposed,
    ...
    final List<Node> proposedChildren = proposed.childNodes();
    // Check proposed elements and compare to originals to find out if we
    // need to add or replace elements
    for (int i = 0, n = proposedChildren.size(); i < n; i++) {
      final Node node = proposedChildren.get(i);
      if (node != null && node instanceof Element) {
        final Element proposedElement = (Element) node;
        final String proposedId = proposedElement.id();
        // Only proposed elements with
        // an id will be considered
        if (proposedId.length() != 0) {
          final Element originalElement = original.getElementById( proposedId );
          // Insert proposed element given the original document has
          // no element with a matching id
          if (null == originalElement) {
            Elements placeHolders = original.getElementsByAttribute( "util:placeholder" );
            if (!placeHolders.isEmpty()) { 
              // Insert right before place holder if we can find it
              placeHolders.get( 0 ).parent().before( proposedElement );
                  original.getOwnerDocument().importNode(proposedElement, false), placeHolders.get( 0 ));
            }
          ...
          // a matching id
            ...
            // Get element hash code
            final String originalElementHashCode = originalElement.getAttribute(hashCodeAttrName);

            // Only act if a hash code exists
            if (originalElementHashCode.length() > 0) {
              ...
              if ("?".equals(originalElementHashCode)
                ...
                if (!equalElements(originalElement, proposedElement)) {
                  ...
                  for (int x = 0; x < childs.getLength(); x++) {
                    ...
                    if (childNode != null && childNode.getNodeType() == Node.ELEMENT_NODE) {
                      ...
                      String zAttribute = child.getAttribute(hashCodeAttrName);
                ...
                if ("?".equals(originalElementHashCode)) {
                  originalElement.setAttribute(hashCodeAttrName,
              ...
              // element as z="user-managed"
                ...
                if (!originalElementHashCode.equals("user-managed")) {
                  originalElement.setAttribute(hashCodeAttrName, "user-managed");
                  originalDocumentChanged = true;
                }
              }
            }
  ...
  private static boolean checkNamespaces(final Document original, final Document proposed) {
    ...
    W3CDom domHelper = new W3CDom();
    final NamedNodeMap nsNodes = domHelper.fromJsoup( proposed ).getAttributes();
    org.w3c.dom.Document originalW3C = domHelper.fromJsoup( original );
    for (int i = 0; i < nsNodes.getLength(); i++) {
      if (0 == originalW3C.getDocumentElement().getAttribute(nsNodes.item(i).getNodeName()).length()) {
        originalW3C.getDocumentElement().setAttribute(nsNodes.item(i).getNodeName(),
  ...
  public static boolean compareDocuments(final Document original, final Document proposed) {
    return compareDocuments(original, proposed, false);
  }
   ...
   */
  public static boolean compareDocuments(final Document original, final Document proposed,
      final boolean isThymeleafTemplate) {
    boolean originalDocumentAdjusted = checkNamespaces(original, proposed);
    hashCodeAttrName = isThymeleafTemplate ? "data-z" : "z";
    originalDocumentAdjusted |=
        addOrUpdateElements(Selector.select( ":root", original ) , Selector.select( ":root", proposed ),
            originalDocumentAdjusted);
    originalDocumentAdjusted |=
  ...
  /**
   * This method will compare the original document with the proposed document
   * and return the merged document. Adjustments are only made if new elements 
   * or attributes are proposed. Changes to the order of attributes or 
   * elements in the original document will not result in an adjustment.
   *
   * @param original document as read from the file system
   * @param proposed document as determined by the JspViewManager
   * @return true if the document was adjusted, otherwise false
   */
  public static Document compareAndReturnMergedDocument(final Document original,
      final Document proposed, final boolean isThymeleafTemplate) {
    boolean originalDocumentAdjusted = checkNamespaces(original, proposed);
    hashCodeAttrName = isThymeleafTemplate ? "data-z" : "z";
    originalDocumentAdjusted |=
      ...
      if (node != null && node.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (originalId.length() != 0) {
          ...
          if (null == proposedElement
              && (originalElement.getAttribute(hashCodeAttrName).equals(
                  calculateUniqueKeyFor(originalElement)) || originalElement.getAttribute(
                  hashCodeAttrName).equals("?"))) {
   ...
   */
  private XmlRoundTripJsoupUtils() {}
}
